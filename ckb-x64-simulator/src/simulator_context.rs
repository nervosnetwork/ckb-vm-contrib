    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::CloseWait(pid, fd) => {
                write!(f, "Close(pid: {}, fd: {})", u64::from(pid.clone()), u64::from(fd.clone()))
            }
...
impl ProcStatus {
    fn read_wait(&self) -> Option<(&ProcID, &Fd, &usize, &[u8])> {
        if let Self::ReadWait(pid, fd, len, buf, _) = self { Some((pid, fd, len, buf)) } else { None }
    }
    fn read_wait_mut(&mut self) -> Option<(&ProcID, &mut Fd, &mut usize, &mut Vec<u8>)> {
        if let Self::ReadWait(pid, fd, len, buf, _) = self { Some((pid, fd, len, buf)) } else { None }
    }
    fn write_wait(&self) -> Option<(&ProcID, &Fd, &[u8])> {
        if let Self::WriteWait(pid, fd, buf, _) = self { Some((pid, fd, buf)) } else { None }
    }
...
impl SimContext {
    ...
    pub fn start_process<F: Send + 'static + FnOnce(SimID, ProcID) -> i8>(&mut self, fds: &[Fd], func: F) -> ProcID {
        let parent_id = ProcInfo::id();
        let id = self.process_id_count.next();
        let process = ProcInfo { parent_id: parent_id.clone(), inherited_fds: fds.to_vec(), ..Default::default() };
        ...
        self.process_status.push(ProcStatus::WaitSpawn(id.clone(), false));
    ...
    fn process(&self, id: &ProcID) -> &ProcInfo {
        self.processes.get(id).unwrap_or_else(|| panic!("unknow process id: {:?}", id))
    }
    fn process_mut(&mut self, id: &ProcID) -> &mut ProcInfo {
        self.processes.get_mut(id).unwrap_or_else(|| panic!("unknow process id: {:?}", id))
    }
    ...
    fn process_io(&mut self, fd: Option<&Fd>) {
        ...
        for i in 0..self.process_status.len() {
            if let Some((_pid, rfd, rlen, _rbuf)) = self.process_status[i].read_wait() {
                if rlen != &0 {
                    ...
                    if let Some(w_pos) = self.process_status.iter().position(|status| match status {
        ...
        update_rw.iter().for_each(|(r_pos, w_pos, is_close)| {
            if *is_close {
                let (_, _rfd, rlen, _rbuf) = self.process_status[*r_pos].read_wait_mut().expect("Unknow error");
                *rlen = 0;
            } else {
                let wbuf =
                    self.process_status[*w_pos].write_wait().map(|(_, _, buf)| buf.to_vec()).expect("unknow error");

                // Update Read Status
                let (_, _rfd, rlen, rbuf) = self.process_status[*r_pos].read_wait_mut().expect("Unknow error");
                let copy_len = (*rlen).min(wbuf.len());
                rbuf.extend_from_slice(&wbuf[..copy_len]);
                *rlen -= copy_len;

                // Update Write Status
                let (_, _wfd, wbuf) = self.process_status[*w_pos].write_wait_mut().expect("unknow error");
        ...
        if let Some((_pid, index)) = notify_items.pop_first() {
            match &self.process_status[index] {
                ...
                ProcStatus::WaitSpawn(pid, _) => {
                    self.process(&self.process(pid).parent_id).scheduler_event.notify();
                }
    ...
    pub fn wait_read(&mut self, fd: Fd, len: usize) -> Event {
        ...
        self.process_status.push(ProcStatus::ReadWait(id, fd.clone(), len, Vec::new(), dbg_id));
    ...
    pub fn wait_write(&mut self, fd: Fd, buf: &[u8]) -> Event {
        ...
        let dbg_id = self.dbg_status_count;
        self.dbg_status_count += 1;
        self.process_status.push(ProcStatus::WriteWait(id, fd.clone(), buf.to_vec(), dbg_id));
    ...
    pub fn close_pipe(&mut self, fd: Fd) -> Result<Event, ()> {
        if !self.has_fd(&fd) {
            ...
            self.process_status.push(ProcStatus::CloseWait(ProcInfo::id(), fd.clone()));
            self.process_io(Some(&fd));
    ...
    fn move_pipe(&mut self, fd: &Fd, pid: ProcID) {
        let f = self.fds.get_mut(fd).unwrap_or_else(|| panic!("unknow fd: {:?}", fd));
        *f = pid;
    }
    fn close_all(&mut self, id: &ProcID) {
        let keys_to_rm: Vec<Fd> = self.fds.iter().filter(|(_k, v)| v == &id).map(|(k, _v)| k.clone()).collect();
        for k in keys_to_rm {
            self.fds.remove(&k);
        }
    ...
    pub fn has_fd(&self, fd: &Fd) -> bool {
        if let Some(pid) = self.fds.get(fd) { &ProcInfo::id() == pid } else { false }
    }
