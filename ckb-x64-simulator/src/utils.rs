...
pub fn get_simulator_path(code_hash: &[u8], hash_type: u8, offset: u32, length: u32) -> Option<String> {
...
impl CkbNativeSimulator {
    pub fn new_by_hash(code_hash: *const u8, hash_type: u8, offset: u32, length: u32) -> Self {
        let sim_path =
            get_simulator_path(unsafe { std::slice::from_raw_parts(code_hash, 32) }, hash_type, offset, length);
    ...
    pub fn ckb_std_main(self, args: Vec<String>) -> i8 {
        type CkbMainFunc<'a> = libloading::Symbol<'a, unsafe extern "C" fn(argc: i32, argv: *const *const i8) -> i8>;
        ...
        unsafe {
            let func: CkbMainFunc = self.lib.get(b"__ckb_std_main").expect("load function : __ckb_std_main");
            func(argc as i32, argv.as_ptr())
        }
    }

    pub fn update_script_info(&self, tx_ctx_id: SimID, pid: ProcID) {
        type SetScriptInfo<'a> =
            libloading::Symbol<'a, unsafe extern "C" fn(ptr: *const c_void, tx_ctx_id: u64, pid: u64)>;

        unsafe {
            let func: SetScriptInfo = self.lib.get(b"__set_script_info").expect("load function : __update_spawn_info");
