...
use ckb_types::{
    bytes::Bytes,
    core::{Capacity, HeaderView, cell::CellMetaBuilder},
    packed::{self, Byte32, CellInput, CellOutput, Script},
    prelude::*,
};
use constants::{
    CELL_FIELD_CAPACITY, CELL_FIELD_DATA_HASH, CELL_FIELD_LOCK, CELL_FIELD_LOCK_HASH, CELL_FIELD_OCCUPIED_CAPACITY,
    CELL_FIELD_TYPE, CELL_FIELD_TYPE_HASH, CKB_INDEX_OUT_OF_BOUND, CKB_ITEM_MISSING, CKB_SUCCESS,
    HEADER_FIELD_EPOCH_LENGTH, HEADER_FIELD_EPOCH_NUMBER, HEADER_FIELD_EPOCH_START_BLOCK_NUMBER, INPUT_FIELD_OUT_POINT,
    INPUT_FIELD_SINCE, SOURCE_CELL_DEP, SOURCE_GROUP_CELL_DEP, SOURCE_GROUP_HEADER_DEP, SOURCE_GROUP_INPUT,
...
lazy_static! {
    static ref TRANSACTION: MockTransaction = {
        ...
        let repr_mock_tx: ReprMockTransaction = serde_json::from_str(&tx_content).expect("parse tx file");
...
fn assert_vm_version() {
    if SETUP.vm_version == 0 {
        panic!("Currently running setup vm_version({}) not support this syscall", SETUP.vm_version);
    }
}
...
pub extern "C" fn ckb_exec_cell(
    ...
    let sim_path = utils::get_simulator_path(utils::to_array(code_hash, 32), hash_type, offset, length);
...
#[unsafe(no_mangle)]
pub extern "C" fn ckb_load_cell(ptr: *mut c_void, len: *mut u64, offset: u64, index: u64, source: u64) -> c_int {
...
#[unsafe(no_mangle)]
pub extern "C" fn ckb_load_input(ptr: *mut c_void, len: *mut u64, offset: u64, index: u64, source: u64) -> c_int {
...
#[unsafe(no_mangle)]
pub extern "C" fn ckb_load_header(ptr: *mut c_void, len: *mut u64, offset: u64, index: u64, source: u64) -> c_int {
...
#[unsafe(no_mangle)]
pub extern "C" fn ckb_load_witness(ptr: *mut c_void, len: *mut u64, offset: u64, index: u64, source: u64) -> c_int {
...
pub extern "C" fn ckb_load_cell_by_field(
    ...
    match field {
        ...
        CELL_FIELD_OCCUPIED_CAPACITY => {
            let data = cell_meta.occupied_capacity().expect("capacity error").as_u64().to_le_bytes();
            store_data(ptr, len, offset, &data[..]);
        }
        CELL_FIELD_LOCK => {
            let lock = cell.lock();
...
pub extern "C" fn ckb_load_header_by_field(
    ...
    let value = match field {
        ...
        HEADER_FIELD_EPOCH_START_BLOCK_NUMBER => header.number().checked_sub(epoch.index()).expect("Overflow!"),
...
#[unsafe(no_mangle)]
pub extern "C" fn ckb_load_cell_data(ptr: *mut c_void, len: *mut u64, offset: u64, index: u64, source: u64) -> c_int {
...
unsafe extern "C" {
    unsafe fn simulator_internal_dlopen2(
...
fn rs_simulator_internal_dlopen2(
    ...
    unsafe {
        simulator_internal_dlopen2(native_library_path, code, length, aligned_addr, aligned_size, handle, consumed_size)
    }
}
...
pub extern "C" fn ckb_dlopen2(
    ...
    let filename = SETUP.native_binaries.get(&key).expect("cannot locate native binary!");
...
fn fetch_cell(index: u64, source: u64) -> Result<(CellOutput, Bytes), c_int> {
    match source {
        ...
        SOURCE_OUTPUT => {
            TRANSACTION.tx.raw().outputs().get(index as usize).ok_or(CKB_INDEX_OUT_OF_BOUND).map(|output| {
                (output, TRANSACTION.tx.raw().outputs_data().get(index as usize).expect("cell data mismatch").unpack())
            })
        }
        ...
        SOURCE_GROUP_INPUT => {
            ...
            indices.get(index as usize).ok_or(CKB_INDEX_OUT_OF_BOUND).and_then(|actual_index| {
                TRANSACTION
                    .mock_info
                    .inputs
                    .get(*actual_index)
...
fn fetch_input(index: u64, source: u64) -> Result<CellInput, c_int> {
    match source {
        SOURCE_INPUT => TRANSACTION.tx.raw().inputs().get(index as usize).ok_or(CKB_INDEX_OUT_OF_BOUND),
        ...
        SOURCE_GROUP_INPUT => {
            ...
            indices
                .get(index as usize)
                .ok_or(CKB_INDEX_OUT_OF_BOUND)
                .and_then(|actual_index| TRANSACTION.tx.raw().inputs().get(*actual_index).ok_or(CKB_INDEX_OUT_OF_BOUND))
        }
...
fn find_header(hash: Byte32) -> Option<HeaderView> {
    TRANSACTION.mock_info.header_deps.iter().find(|header| header.hash() == hash).cloned()
}
...
fn fetch_header(index: u64, source: u64) -> Result<HeaderView, c_int> {
    match source {
        ...
        SOURCE_CELL_DEP => TRANSACTION
            ...
            .and_then(|header_hash| find_header(header_hash).ok_or(CKB_ITEM_MISSING)),
        SOURCE_HEADER_DEP => {
            TRANSACTION.mock_info.header_deps.get(index as usize).cloned().ok_or(CKB_INDEX_OUT_OF_BOUND)
        }
        SOURCE_GROUP_INPUT => {
            let (indices, _) = fetch_group_indices();
            indices.get(index as usize).ok_or(CKB_INDEX_OUT_OF_BOUND).and_then(|actual_index| {
...
fn fetch_witness(index: u64, source: u64) -> Option<packed::Bytes> {
    match source {
        ...
        SOURCE_GROUP_INPUT => {
            let (indices, _) = fetch_group_indices();
            indices.get(index as usize).and_then(|actual_index| TRANSACTION.tx.witnesses().get(*actual_index))
        }
        SOURCE_GROUP_OUTPUT => {
            let (_, indices) = fetch_group_indices();
            indices.get(index as usize).and_then(|actual_index| TRANSACTION.tx.witnesses().get(*actual_index))
...
fn fetch_current_script() -> Script {
    let cell = if SETUP.is_output {
        TRANSACTION.tx.raw().outputs().get(SETUP.script_index as usize).expect("running script index out of bound!")
    } else {
        TRANSACTION
            .mock_info
            .inputs
    ...
    if SETUP.is_lock_script { cell.lock() } else { cell.type_().to_opt().unwrap() }
}
