...
impl Default for GlobalData {
    fn default() -> Self {
        ...
        Self { tx_ctx: [(0.into(), SimContext::default())].into(), tx_ctx_id_count: 1.into() }
    }
}
...
impl GlobalData {
    ...
    pub fn get_tx(&self, id: &SimID) -> &SimContext {
        self.tx_ctx.get(id).unwrap_or_else(|| panic!("unknow tx context: {:?}", id))
    }
    pub fn get_tx_mut(&mut self, id: &SimID) -> &mut SimContext {
        self.tx_ctx.get_mut(id).unwrap_or_else(|| panic!("unknow mut tx context: {:?}", id))
    }
}
