pub mod generated {
    pub mod traces {
        ...
        pub static DESCRIPTOR_POOL: Lazy<DescriptorPool> =
            Lazy::new(|| DescriptorPool::decode(include_bytes!("generated/file_descriptor_set.bin").as_ref()).unwrap());

        include!("generated/generated.traces.rs");
    }
}
...
fn parse_from_cell(s: &str) -> Result<bool, String> {
    if s == "t" || s == "true" { Ok(true) } else { Ok(false) }
}
...
enum PartialSyscallContent {
    ...
    IoDataAsCode { data_addr: u64, input_length: u64 }, // Actual binary code.
    ...
    Ok(if let Ok(code) = machine.registers()[A7].to_u64().try_into() {
        Some(match code {
            ...
            SyscallCode::LoadCellDataAsCode => {
                let data_addr = machine.registers()[A0].to_u64();
                let input_length = machine.registers()[A3].to_u64();
