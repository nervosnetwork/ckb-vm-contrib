    ...
    if let Some(script_group) = script_group {
        ...
        for (key, (locator, trace)) in traces {
            ...
            if cli.text {
                ...
                let dmessage =
                    prost_reflect::DynamicMessage::decode(descriptor, &bytes[..]).expect("decode dynamic message");
                bytes = dmessage
                    .to_text_format_with_options(&prost_reflect::text_format::FormatOptions::new().pretty(true))
                    .into_bytes();
            }
